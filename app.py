...
<style>
  ...
  .profile-container h1{
      color:var(--brand-b) !important; font-size:1.35rem !important;
  ...
  /* Avoid horizontal scroll within iframe */
...
def create_quick_action_button(text, url):
    return f'<a href="{html.escape(url)}" target="_blank" class="quick-action-btn">{html.escape(text)}</a>'
...
def arxiv_article_search(query, limit=5):
    ...
    try:
        ...
        for entry in feed.entries:
            title = entry.title
            pdf_links = [l.href for l in entry.links if getattr(l, "type", "") == "application/pdf"]
...
def doaj_article_search(query, limit=5):
    ...
    try:
        ...
        for art in articles:
            ...
            link = (bibjson.get("link", [{}])[0] or {}).get("url", "#")
...
# ==== LLM/FAQ helpers (unchanged logic) ====
def create_payload(prompt: str):
    system_instruction = (
        ...
        # ... (same long instruction you already had) ...
    ...
    if "find books on" in prompt.lower() or "find book on" in prompt.lower():
        ...
        if books:
            for b in books:
                ...
                answer += f"- [{b['title']}]({b['url']}){authors}{pub}{year}\n"
        else:
    ...
    if any(kw in prompt.lower() for kw in article_keywords):
        ...
        if core_results:
            for art in core_results:
                ...
                url = art.get("downloadUrl", (art.get("urls", [{}])[0] or {}).get("url", "#"))
        ...
        if doaj_results:
            for a in doaj_results:
                answer += f"- [{a['title']}]({a['url']}) ({a['year']}) - {a['journal']}\n"
        else:
...
# ==== Auto-size broadcaster: tell parent our content height whenever it changes ====
st.markdown("""
<script>
(function(){
  function sendSize(){
    try{
      ...
      parent.postMessage({ type: 'ashuSize', height: h, width: w }, '*');
    }catch(e){}
  }
  // On load + slight delay + whenever layout changes
